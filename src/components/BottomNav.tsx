...
const tabs: { id: TabType; label: string; icon: typeof Clock }[] = [
  ...
  { id: 'shift', label: 'Смена', icon: BarChart3 },
...
export function BottomNav({ activeTab, onTabChange, rentedCount, availableCount, warehouseCount, isAdmin = false }: BottomNavProps) {
  // Сотруднику доступна вкладка «Смена», админу — «Отчёты» и «Персонал»
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 glass-header safe-area-bottom">
      <div className="flex items-center justify-around h-20 max-w-lg mx-auto px-2 pb-1">
        {visibleTabs.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center gap-1 flex-1 h-16 rounded-2xl transition-colors duration-300 ${
                ...
                <div className="relative">
                  ...
                  {badge > 0 && (
                    <motion.span 
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="absolute -top-2 -right-3 min-w-[20px] h-[20px] flex items-center justify-center rounded-full bg-primary text-primary-foreground text-[11px] font-bold px-1 border-2 border-background"
                    >
                      {badge}
                    </motion.span>
                  )}
